...
while ($row=$stmt->fetch()) { 
	if ($row['statut']=="oui") {
		...
		ferme_bdd();
		exit();
	} else { // $row['statut']=="sibesoin"
		if ($_POST['action']=='avecclick') {
			$stmt=$dbh->prepare('UPDATE GESTIONCRENEAUX SET statut="oui"  WHERE idcreneau=? AND idstaff=?');
			$stmt->bindParam(1,$id);
			$stmt->bindParam(2,$idstaffeur);
			$stmt->execute();		
			$stmt3=$dbh->prepare('SELECT nbstaff FROM CRENEAUX WHERE id=?');
...
if ($_POST['action']=='avecclick') {
   $stmt=$dbh->prepare('INSERT INTO GESTIONCRENEAUX (idcreneau,idstaff,statut) VALUES (?,?,"sibesoin")');
   $stmt->bindParam(1,$id);
   $stmt->bindParam(2,$idstaffeur);
   $stmt->execute();
   echo json_encode(["sibesoin",-1]);
